...
def handler(event, context):
  ...
  owner, repo, branch, filename, start_line, end_line = validate_link(github_link)
  request = requests.get("https://api.github.com/repos/%s/%s/contents/%s?ref=%s" % (owner, repo, filename, branch)).json()
